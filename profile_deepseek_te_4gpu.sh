#!/bin/bash
# =============================================================================
# profile_deepseek_te_4gpu.sh
#
# Multi-GPU (EP=2, FSDP=2 → 4 GPUs) profiling of TE router + permutation with
...
#
# Usage:
#   bash profile_deepseek_te_4gpu.sh                    # profile only
#   bash profile_deepseek_te_4gpu.sh --dump-hlo         # profile + HLO dump
#   bash profile_deepseek_te_4gpu.sh --launch-xprof     # profile + launch xprof
#   bash profile_deepseek_te_4gpu.sh --warmup 5         # more warmup steps
#   bash profile_deepseek_te_4gpu.sh --batch 4          # larger batch
# =============================================================================

set -euo pipefail

MAXTEXT_DIR="${MAXTEXT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
PROFILE_BASE_DIR="${MAXTEXT_DIR}/xprof_profiles_deepseek_4gpu"
EP=2
FSDP=2
WARMUP_STEPS=3
PROFILE_STEPS=1
...
echo "  ${TOTAL_GPUS} GPUs | EP=${EP} FSDP=${FSDP} | sparse_matmul | ring-of-experts"
echo "=============================================="
...
run_profile() {
    ...
    echo ""
    echo "======================================================"
    echo "  PROFILING: ${run_name}  (${TOTAL_GPUS} GPUs: EP=${EP}, FSDP=${FSDP})"
...
echo ""
echo "=============================================="
echo "Profiling Complete  (${TOTAL_GPUS} GPUs: EP=${EP}, FSDP=${FSDP})"
echo "=============================================="
echo ""
...
echo ""
echo "To view:"
echo "  xprof --port 8794 ${TE_GMM_DIR}      # Run 1: TE + TE GMM"
echo "  xprof --port 8795 ${MT_DIR}           # Run 2: MT + ragged_dot"
echo "  xprof --port 8796 ${TE_RAGGED_DIR}    # Run 3: TE + ragged_dot"
echo "  Then SSH tunnel: ssh -L 8794:localhost:8794 -L 8795:localhost:8795 -L 8796:localhost:8796 <host>"
echo "  Browser:"
echo "    http://localhost:8794  TE + TE GMM       → Tools > trace_viewer"
echo "    http://localhost:8795  MT + ragged_dot    → Tools > trace_viewer"
echo "    http://localhost:8796  TE + ragged_dot    → Tools > trace_viewer"
echo "=============================================="
