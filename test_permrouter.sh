...
te_permutation_impl=false \
max_segments_per_seq=32 \
te_permutation_align_size=0 \
...
MAXTEXT_DIR=/mnt/tdophung/prenyx_lustre_home/maxtext test-maxtext.sh -b 2 --model-name=mixtral-8x7b --attn-type=cudnn_flash_te --remat-policy=minimal_with_context --steps=30 --data-parallel=1 --fsdp=$FSDP --tensor-parallel=1 --tensor-sequence-parallel=1 -a "$ADDITIONAL_ARGS"
